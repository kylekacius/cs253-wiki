...
#templating config, finds template folder and loads templates
...
#takes in the plain text username and password from the form, and check
#if a salt has been created yet, then creates salt, and hashes the pw, returns both to be stored in db
...
#function determines if the password is valid when logging the user in
#pulls out the salt and compares the hash stored in the db with the new hash created using the salt
...
#An entity can also have an optional parent entity. Parent-child relationships form entity groups, 
#which are used to control transactionality and data locality in the Datastore. An application creates
#a parent-child relationship between two entities by passing the parent entity to the child entity's constructor, as the parent argument.
